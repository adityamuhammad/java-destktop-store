...
public class mstr_customer {
    ...
    public String cust_id, nama, alamat, telp;
    ...
    public void hapusData(String kode) {
        ...
        try {
            ...
            String _sql = "DELETE FROM store_akhtar.mstr_customer WHERE mstr_customer.cust_id = '" +cust_id + "'";
    ...
    public void Cari(String cust_id) {
        ...
        try {
            _Akses = "";
            _Cnn = DBO.getConnection();
            String SQL = "SELECT * FROM mstr_customer WHERE cust_id = '" + pilih_cust_id + "'";
    ...
    public void simpanOrUpdate(){
        try {
            ...
            _Cnn = DBO.getConnection();
            if (_Akses.equals("")) {
                _sql = "INSERT INTO store_akhtar.mstr_customer (cust_id, nama, alamat, telp) VALUES ('?','?','?','?')";
                System.out.println(_sql);
            } else {
                _sql = "UPDATE store_akhtar.mstr_customer SET "
                        + "cust_id = '"+cust_id+ "',"
            ...
            PreparedStatement prs = _Cnn.prepareStatement(_sql);
            prs.setString(1, this.cust_id);
            prs.setString(1, this.nama);
            prs.setString(1, this.alamat);
            prs.setString(1, this.telp);
            prs.executeUpdate();
            System.out.println(_sql);
            prs.close();
        } catch (Exception err) {
