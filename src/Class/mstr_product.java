...
public class mstr_product {
    ...
    public void hapusData(String kode) {
        ...
        try {
            ...
            String _sql = "DELETE FROM store_akhtar.mstr_product WHERE mstr_product.prd_id = '"+prd_id+ "'";
    ...
    public void simpanData() {
        try {
            String _sql = "";
            _Cnn = DBO.getConnection();
            _sql = "INSERT INTO mstr_product(prd_id, nama,satuan,harga) VALUES (?,?,?,?)";
            PreparedStatement prs = _Cnn.prepareStatement(_sql);
    ...
    public void Cari(String prd_id) {
        ...
        try {
            _Akses = "";
            _Cnn = DBO.getConnection();
            String SQL = "SELECT * FROM mstr_product WHERE prd_id = '" +pilih_prod_id + "'";
    ...
    public void simpanOrUpdate(){
        try {
            ...
            _Cnn = DBO.getConnection();
            if (_Akses.equals("")) {
                _sql = "INSERT INTO store_akhtar.mstr_produk (prd_id,nama,satuan,harga)"
                        + "VALUES ('?','?','?','?')";
                System.out.println(_sql);
            } else {
                _sql = "UPDATE store_akhtar.mstr_product SET "
